...
#'  3' Exon events and renders the two-region sequence map.
#'
#' @param events Data frame of rMATS a3ss events.
...
#' @export
three_prime_sequence_map <- function(events, sequence,
                                          genome     = NULL,
  ...
  wrap_sm_errors("3' sequence map", {
    ...
    if (missing(events)) {
      abort_invalid_arg(c(
        ...
        "i" = "Supply a data frame of rMATS a3ss events."
      ))
    }
    if (missing(sequence)) {
      abort_invalid_arg(c(
