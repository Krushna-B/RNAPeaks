FilterBed<-function(bed,chr,start,end,strand,omit=c(),collapse){
  bed<-bed[which(as.character(bed$chr)==chr & bed$start>=start & bed$end<=end & bed$strand==strand),]
  ...
  bed_gr<-GenomicRanges::makeGRangesListFromDataFrame(bed,keep.extra.columns = T,split.field="tag")
