.splicing_map_worker <- function(mats_data,
                                  bed_file,
                                  bins_fn,
                                  n_bins,
                                  plot_fn,
  ...
  mats_data$chr <- toupper(sub("^chr", "", mats_data$chr, ignore.case = TRUE))
  ...
  apply_moving_avg <- function(freq_vec) {
    ...
    for (b in seq_len(n_bins)) {
      ...
      bin_end   <- b * bin_width
      result[bin_start:bin_end] <- slider::slide_dbl(
        freq_vec[bin_start:bin_end], mean,
        .before = half_window, .after = half_window, .complete = FALSE
      )
    }
    result
  }
  ...
  if ("Control" %in% groups) {
    ...
    if (n_controls == 0) {
      ...
      for (iter in seq_len(control_iterations)) {
        ...
        iteration_results[[iter]] <- apply_moving_avg(overlap_counts / sample_size)
      }
  ...
  if (length(missing_groups) > 0) {
    ...
    zero_rows <- do.call(rbind, lapply(missing_groups, function(g) {
      data.frame(
        global_position = seq_len(n_bins * bin_width),
        overlap_count   = 0L,
        frequency       = 0,
    ...
    combined_data <- dplyr::bind_rows(combined_data, zero_rows)
  }
