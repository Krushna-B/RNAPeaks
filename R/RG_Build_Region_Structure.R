Build_Region_Structure <- function(
  ...
  Range<-GenomicRanges::GRanges(seqnames = Chr,
  ...
  # Keep a global copy for debugging/inspection
  unclipped_region_of_Gene <<- unclipped_region
...
#Clips the parts of exons, introns, UTRS not included in exact given region
  clip_overlapping_parts <- function(gtf_rows_df, range_gr, Strand = NULL) {
    ...
    gr <- makeGRangesFromDataFrame(gtf_rows_df, keep.extra.columns = TRUE)
    ...
    clip <- GenomicRanges::pintersect(gr[S4Vectors::queryHits(overlaps)],
