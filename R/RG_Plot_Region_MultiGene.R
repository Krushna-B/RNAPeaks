...
Get_Multi_Plot_by_Region<-function(Chr,
  ...
  #Fallback gene frame used by Prepare_Bed if none provided so that Prepare_bed knows start and end positions
  if (is.null(gene)) {
...
#Gene IDs present in window one transcript for each geneID
  ...
  ALL_GENE_STRUCTS <<- gene_structs[[1]]$Gene_s
  ...
  combined_gene <- data.frame(
    seqnames  = Gene_s_all$seqnames[1],        # assume all on same chr
  ...
  #TODO: Arrows on all of the genes
  Intron_all$mid_y <- (Intron_all$y_start + Intron_all$y_end) / 2

  #Make arrows
  arrow_df_all <- make_intron_arrows(introns_all$Introns_Positions, Gene_s_all$strand[1], 20)
  ...
  label_df <- if (nrow(Gene_s_all)) {
    # y per lane
    agg <- aggregate(y_start ~ gene_id, Gene_s_all, mean)
  ...
  #Clean Gene Names to consistent vlaues if they are empty
  clean_gene_names <- function(df) {
    # make sure it's character, not factor
    df$gene_name <- as.character(df$gene_name)

    # anything empty / space / literal "NA" → treat as missing
...
#Builds vertical offset based off pre-specifed order of the genes, chronological order
