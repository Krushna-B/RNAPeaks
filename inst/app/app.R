...
# Resolve data file paths: prefer system.file() (installed package) and fall
# back to a relative "data/" path (standalone deployment or devtools context).
pkg_data <- function(f) {
  p <- system.file("data", f, package = "RNAPeaks")
  if (nzchar(p)) p else file.path("data", f)
}

# Load sample data
load(pkg_data("sample_bed.rda"))
load(pkg_data("sample_se.mats.rda"))

# Pre-load GTF at startup so it's ready before any user request
load(pkg_data("gtf_human.rda"))
